...
export default function SeoPage({ page, locale }) {
  ...
  return <div className={styles.page} data-motion-root>
    ...
    <main>
      ...
      {articleLayout && copy.sources?.length > 0 && <section className={styles.sources}><div className={styles.container}><p className={styles.kicker}>{locale === "tr" ? "Kaynaklar" : "Sources"}</p><h2>{locale === "tr" ? "Kontrol edilen birincil kaynaklar" : "Primary sources reviewed"}</h2><div>{copy.sources.map(([label, url]) => <a href={url} target="_blank" rel="noreferrer" key={url}><span>{label}</span><b>↗</b></a>)}</div></div></section>}
