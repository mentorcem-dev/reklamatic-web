...
function experiencePreset(page, locale) {
  ...
  const base = locale === "tr"
    ? { kicker: "REKLAMATIC DAĞITIM SİSTEMİ", title: "Kısa video üretiminden gerçek dağıtıma.", text: "300 milyondan fazla geçmiş görüntülenme deneyimini; aktif clipper ağı, içerik üretimi ve ölçülebilir kampanya yönetimiyle birleştiriyoruz.", metrics: [["300M+", "geçmiş platform view"], ["50–200", "hesaplı dağıtım"], ["1K", "clipper ödeme birimi"]], flow: ["Kaynak", "Klip", "Hesap", "View", "Rapor"] }
    : { kicker: "REKLAMATIC DISTRIBUTION SYSTEM", title: "From short-form production to real distribution.", text: "We combine more than 300 million historical platform views with an active clipper network, content production and measurable campaign operations.", metrics: [["300M+", "historical platform views"], ["50–200", "account distribution"], ["1K", "clipper payout unit"]], flow: ["Source", "Clip", "Account", "Views", "Report"] };
...
export default function SeoPage({ page, locale }) {
  ...
  return <div className={styles.page} data-motion-root>
    ...
    <footer className={styles.footer}><div className={styles.container}><div><Link href={home(locale)}><Logo /></Link><p>{locale === "tr" ? "Uygulama, müzik ve yayıncılar için yönetilen clipping kampanyaları." : "Managed clipping campaigns for apps, music and publishers."}</p></div><div><strong>{locale === "tr" ? "Çalışma yolları" : "Work with us"}</strong><Link href={localPath(locale, seoPages.find((item) => item.key === "brands").paths[locale])}>{locale === "tr" ? "Reklam ver" : "For brands"}</Link><Link href={localPath(locale, seoPages.find((item) => item.key === "clippers").paths[locale])}>{locale === "tr" ? "Clipper ilgisi" : "For clippers"}</Link><Link href={localPath(locale, seoPages.find((item) => item.key === "pricing").paths[locale])}>{locale === "tr" ? "Kampanya bütçesi" : "Pricing"}</Link></div><div><strong>{locale === "tr" ? "Keşfet" : "Resources"}</strong><Link href={localPath(locale, seoPages.find((item) => item.key === "campaigns").paths[locale])}>{locale === "tr" ? "Nasıl çalışır?" : "How it works"}</Link><Link href={localPath(locale, seoPages.find((item) => item.key === "cases").paths[locale])}>{locale === "tr" ? "Kampanya örnekleri" : "Campaign examples"}</Link><Link href={localPath(locale, seoPages.find((item) => item.key === "faq").paths[locale])}>{locale === "tr" ? "Sık sorulanlar" : "FAQ"}</Link></div><div><strong>{locale === "tr" ? "İletişim" : "Contact"}</strong><a href="mailto:info@reklamatic.ai">info@reklamatic.ai</a><a href="tel:+905302312947">+90 530 231 29 47</a><Link href={locale === "tr" ? "/privacy" : "/en/privacy"}>{locale === "tr" ? "Gizlilik" : "Privacy"}</Link></div></div></footer>
  </div>;
}
